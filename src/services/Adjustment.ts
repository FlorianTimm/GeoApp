...
export class Adjustment {
    ...
    private x_measurements: { measure: number, component: 'o' | 'ih' }[] = [];
    private x_measurements_pos: { [measure: number]: { o?: number, ih?: number } } = {};
    private l_measurements: { measure: number, entry: number, component: 'hz' | 'v' | 'dist' }[] = [];
    private x_points: { nr: string, component: 'x' | 'y' | 'z' }[] = [];
    private x_points_pos: { [nr: string]: { x?: number, y?: number, z?: number } } = {};
    private l_points: { nr: string, component: 'x' | 'y' | 'z' }[] = [];
    ...
    private createX0Vector() {
        ...
        for (let nr in this.points) {
            ...
            if (c.x !== undefined) {
                this.x0.push(c.x);
                this.x_points_pos[nr].x = this.x0.length - 1;
                this.x_points.push({ nr: nr, component: 'x' });
                this.x_isAngle.push(false);
            }
            if (c.y !== undefined) {
                this.x0.push(c.y);
                this.x_points_pos[nr].y = this.x0.length - 1;
                this.x_points.push({ nr: nr, component: 'y' });
                this.x_isAngle.push(false);
            }
            if (c.z !== undefined) {
                this.x0.push(c.z);
                this.x_points_pos[nr].z = this.x0.length - 1;
                this.x_points.push({ nr: nr, component: 'z' });
                this.x_isAngle.push(false);
            }
        }
        ...
        this.measurements.forEach((m, i) => {
            if (m.type == 'theodolite' && m instanceof TheodoliteMeasure) {
                ...
                this.x_measurements[i] = { measure: i, component: 'o' };
/*
                if (m.instrumentHeight !== undefined) {
                    this.x0.push(m.instrumentHeight);
                    this.x_isAngle.push(false);
                    entry.ih = this.x0.length - 1;
                    this.x_measurements[i] = { measure: i, component: 'ih' };
    ...
    private filterImpossibleMeasurements() {
        let A_sum = sum(this.A, 0) as unknown as number[];
        this.x_filter = A_sum.map((x) => x === 0)
        let A_sum2 = sum(this.A, 1) as unknown as number[];
        this.l_filter = A_sum2.map((x) => x === 0)
        return this.x_filter, this.l_filter;
    }
    ...
    private createA_l0_dl_l() {
                ...
                m.measures.forEach((measure, n) => {
                    ...
                    if (measure.hz !== undefined && this.x_measurements_pos[i].o !== undefined) {
                        ...
                        */
                        ...
                        azi -= this.x0[this.x_measurements_pos[i].o];
                        azi = gonBetween0And400(azi);
                        this.l0.push(azi);
                        this.l.push(measure.hz);
                        this.l_measurements.push({ measure: i, entry: n, component: 'hz' });
                    ...
                    if (measure.distance !== undefined) {
                        ...
                        row[ty] = dy / dist;
                        this.l0.push(dist);
                        this.l_measurements.push({ measure: i, entry: n, component: 'dist' });
        ...
        // else: 'manual' | 'gps' | 'map' | 'import' | 'transform' 
            ...
            if (c.x !== undefined && this.x_points_pos[nr].x !== undefined) {
                ...
                this.l0.push(this.x0[this.x_points_pos[nr].x]);
                this.l.push(c.x);
                this.l_points.push({ nr: nr, component: 'x' });
                this.l_isAngle.push(false);
            ...
            if (c.y !== undefined && this.x_points_pos[nr].y !== undefined) {
                ...
                this.l0.push(this.x0[this.x_points_pos[nr].y]);
                this.l.push(c.y);
                this.l_points.push({ nr: nr, component: 'y' });
                this.l_isAngle.push(false);
                this.p.push(c.accuracy);
                this.A.push(row);
            }
            if (c.z !== undefined && this.x_points_pos[nr].z !== undefined) {
                ...
                this.l0.push(this.x0[this.x_points_pos[nr].z]);
                this.l.push(c.z);
                this.l_points.push({ nr: nr, component: 'z' });
                this.l_isAngle.push(false);
                this.p.push(c.accuracy);
                this.A.push(row);
            }
    ...
    private calculate() {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            this.x_isAngle.forEach((x, i) => {
                if (x) {
                    this.x0[i] = gonBetween0And400(this.x0[i]);
